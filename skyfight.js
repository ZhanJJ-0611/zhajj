...
function openSkyFight() {
  ...
  function drawHUD() {
    ...
    ctx.fillText('♥'.repeat(pHp) + '♡'.repeat(3 - pHp), 8, 15)

    // 分数 & 等级
    ctx.fillStyle = 'white'; ctx.font = 'bold 11px system-ui'; ctx.textAlign = 'right'
    ctx.fillText(`${score} 分   Lv.${level}`, W - 8, 15)
  }
  ...
  function drawDeadOverlay() {
    ...
    ctx.fillStyle = '#f87171'; ctx.font = 'bold 21px system-ui'
    ctx.fillText('飞机被击落！', W / 2, H / 2 - 22)
    ctx.fillStyle = 'rgba(255,255,255,0.78)'; ctx.font = '13px system-ui'
    ctx.fillText(`得分：${score} 分`, W / 2, H / 2 + 6)
