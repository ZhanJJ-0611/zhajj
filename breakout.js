...
function openBreakout() {
  ...
  function drawHUD() {
    ...
    ctx.textAlign = 'right'
    ctx.fillText(`?? ${TOTAL - broken} ?`, W - 8, 15)
  }
  ...
  function drawOverlay() {
    ...
    if (state === 'dead') {
      ...
      ctx.fillText(`??? ${broken} / ${TOTAL} ?`, W / 2, H / 2 + 28)
    } else {
...
function openBreakout() {
  ...
  const BGAPX = 3
  const BGAPY = 4
  const BOFFX = (W - (COLS * (BW + BGAPX) - BGAPX)) / 2
  const BOFFY = 28
  ...
  const BALL_R = 5.5
  let bx = W / 2
  let by = PAD_Y - BALL_R - 1
  const initSpeed = 2.55
  let bdx = initSpeed * 0.55
  let bdy = -initSpeed * 0.835
  ...
  function update() {
    ...
    if (
      bdy > 0 &&
      by + BALL_R >= PAD_Y &&
      by + BALL_R <= PAD_Y + PAD_H + Math.abs(bdy) &&
      bx >= padX - BALL_R &&
      bx <= padX + padW + BALL_R
    ) {
      by = PAD_Y - BALL_R
      const hitPos = Math.max(-1, Math.min(1, (bx - (padX + padW / 2)) / (padW / 2)))
      const angle = hitPos * (Math.PI / 3.2)
      const speed = Math.hypot(bdx, bdy)
      bdx = speed * Math.sin(angle)
      bdy = -Math.abs(speed * Math.cos(angle))
      if (Math.abs(bdy) < speed * 0.32) bdy = -speed * 0.32
    }
    ...
    for (const b of bricks) {
      ...
      if (bx + BALL_R > b.x && bx - BALL_R < bRight &&
        ...
        if (b.hp <= 0) {
          ...
          if (broken % 5 === 0) {
            const speed = Math.hypot(bdx, bdy)
            const newSpeed = Math.min(5.1, speed + 0.26)
  ...
  function drawOverlay() {
    ...
    if (state === 'dead') {
      ...
      ctx.fillText(`已打掉 ${broken} / ${TOTAL} 块`, W / 2, H / 2 + 28)
    } else {
      ctx.fillStyle = '#fcd34d'
      ctx.font = 'bold 20px system-ui'
      ctx.textAlign = 'center'
